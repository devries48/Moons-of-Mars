...
namespace MoonsOfMars.SolarSystem
{
    ...
    public class KeplerOrbitData
    {
        ...
        /// </summary>
        public static readonly KeplerVector3d EclipticNormal = new(0, 1, 0);  // 0,0,1

        /// <summary>
        /// Up direction on ecliptic plane (y-axis on xy ecliptic plane).
        /// </summary>
        public static readonly KeplerVector3d EclipticUp = new(0, 0, 1);  // 0,1,0

        /// <summary>
        /// Right vector on ecliptic plane (x-axis on xy ecliptic plane).
        /// </summary>
        public static readonly KeplerVector3d EclipticRight = new(1, 0, 0); //1,0,0
            ...
            double gConst, double period)
        {
            ...
            SemiMinorAxisBasis = KeplerVector3d.Cross(Periapsis, normal);

            MeanAnomaly = meanAnomalyDeg * KeplerOrbitUtils.Deg2Rad;
        ...
        {
            Eccentricity = eccentricity;
            SemiMajorAxisBasis = semiMajorAxis.normalized;
            SemiMinorAxisBasis = semiMinorAxis.normalized;
        ...
        public KeplerVector3d GetCentralPositionAtEccentricAnomaly(double eccentricAnomaly)
        {
            if (Eccentricity < 1.0)
            {
                KeplerVector3d result = new(Math.Sin(eccentricAnomaly) * SemiMinorAxis, -Math.Cos(eccentricAnomaly) * SemiMajorAxis);
                return -SemiMinorAxisBasis * result.x - SemiMajorAxisBasis * result.y;
            }
            else if (Eccentricity > 1.0)
            {
                KeplerVector3d result = new(Math.Sinh(eccentricAnomaly) * SemiMinorAxis, Math.Cosh(eccentricAnomaly) * SemiMajorAxis);
                return -SemiMinorAxisBasis * result.x - SemiMajorAxisBasis * result.y;
            }
            else
            {
